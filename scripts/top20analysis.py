...
if os.path.exists('data/preprocessed/decisions_top20_lemmas.csv'):
    dataframe=pd.read_csv('data/preprocessed/decisions_top20_lemmas.csv')
...
if not preprocessed_data:
    ...
    dataframe.to_csv('data/preprocessed/decisions_top20_lemmas.csv', index=False)
...
def save_lemma_pdf_given_lemma_plots(data, labels):
    for idx, col in enumerate(labels):
        ...
        df_sum_given_genre = df_sum_given_genre.sort_values('Total', ascending=False).head(5)
        ...
        ax.set(xlabel='', ylabel='Γνωμοδοτήσεις(Normalized)')
        plt.xticks(rotation=90)
...
def save_lemma_pdf_given_lemma_subplots(data, labels):
    ...
    for idx, col in enumerate(lemma_columns):
         ...
         df_sum_given_lemma = df_sum_given_lemma.sort_values('Total', ascending=False).head(5)
...
def save_numLemma_pdf_given_lemma_subplots(data, labels):
     ...
     for idx, col in enumerate(lemma_columns):
         ...
         if (idx + 1) % fig_per_decision == 0 or idx == len(lemma_columns) - 1:
             plt.savefig('./Images/results/numLemma_pdf_part{0}.png'.format(format(1+idx//fig_per_decision), bbox_inches='tight'))
             plt.close()
             ax.set(title='Κατανομή για το λήμμα: {0} '.format(col), xlabel='',
                ylabel='Γνωμοδοτήσεις(Normalized)')
             fig = plt.figure(figsize=(20, 30))
...
for idx, col in enumerate(lemma_columns):
    ...
    ax = fig.add_subplot(num_rows, num_cols, idx+1)
    sns.barplot(data=df_numL_given_lemma, x=df_numL_given_lemma.index, y='Total', axes=ax)
    ax.set(title=format(col), xlabel='',
           ylabel='')
plt.tight_layout()
plt.show()
