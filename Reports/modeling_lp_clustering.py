...
# Από τις μέγιστες δυνατές 2 ^ 40 ετικέτες, βλέπουμε ότι υπάρχουν μόνο 807 μοναδικοί συνδυασμοί.
# Θα χρησιμοποιήσουμε κάποια τεχνική ομαδοποίησης Κ-μέσων για να δούμε πόσες από αυτές τις
...
ax.set(title='Overall F1 Score ΣΕΚ (Απώλεια οφειλόμενη σε ομαδοποίηση με 807 Clusters)')
...
lp_cluster_tfidf_LSVC = accuracy(test_y,predictions)
lp_cluster_tfidf_LSVC.to_excel('data/Results/lp_cluster_tfidf_LSVC.xlsx')
###################
pipeline = Pipeline([
...
predictions = multi_class_predict(best_clf, test_X, cluster_center)
print(accuracy(test_y, predictions))
lp_cluster_cv_LSVC = accuracy(test_y,predictions)
lp_cluster_cv_LSVC.to_excel('data/Results/lp_cluster_cv_LSVC.xlsx')
...
pipeline = Pipeline([
                ...
                ('clf', RandomForestClassifier(n_estimators=200, max_depth=20, max_features='sqrt', n_jobs=4))
            ])
...
lp_cluster_raforest = accuracy(test_y, predictions)
lp_cluster_raforest.to_excel('data/Results/lp_cluster_raforest.xlsx')
...
lp_cluster_tfidf_bayes = accuracy(test_y, predictions)
lp_cluster_tfidf_bayes.to_excel('data/Results/lp_cluster_tfidf_bayes.xlsx')
